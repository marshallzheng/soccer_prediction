...
def test_simulate_match_and_stream_websocket_update() -> None:
    with TestClient(app) as client:
        ...
        with client.websocket_connect(f"/ws/matches/{fixture_id}") as ws:
            message = ws.receive_json()

        assert message["fixture_id"] == fixture_id
        ...
        ids = [m["fixture_id"] for m in list_response.json()]
        assert fixture_id in ids

        detail_response = client.get(f"/api/matches/{fixture_id}")
...
def test_match_history_returns_ticks_after_running_briefly() -> None:
    with TestClient(app) as client:
        ...
        history = client.get(f"/api/matches/{fixture_id}/history")
